      ...
      <section className="bg-[#FAFAF7] pt-36 pb-20">
        <div className="mx-auto max-w-[1600px] px-6 lg:px-12">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
            <a href="#openings" className="mt-8 inline-flex items-center gap-2.5 rounded-lg bg-[#1B2B4B] px-6 py-3 font-heading text-[11.5px] font-bold uppercase tracking-[0.09em] text-[#FAFAF7] border border-white/[0.11] hover:bg-[#162340] active:scale-[0.98] transition-all">
      ...
      <section className="bg-[#E8EDF5] py-24">
        <div className="mx-auto max-w-[1600px] px-6 lg:px-12 grid gap-12 lg:grid-cols-2 items-center">
          ...
          <ScrollReveal delay={0.2}>
            <h2 className="font-display text-[clamp(28px,3.8vw,56px)] leading-[1.15] tracking-[-0.02em] text-[#0B1029]">Join the team</h2>
        ...
        <div className="relative z-10 mx-auto max-w-[1600px] px-6 lg:px-12">
          <ScrollReveal>
            <div className="inline-flex items-center gap-2.5 rounded-lg px-3.5 py-1.5" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.10)" }}>
              <span className="font-mono-label text-[10px] tracking-[0.22em] uppercase text-[#C5CDE0]/70">Why Longentis</span>
            </div>
            <h2 className="font-display mt-6 text-[clamp(28px,3.8vw,56px)] leading-[1.15] tracking-[-0.02em] text-[#E8E2D9]">What you can expect</h2>
          </ScrollReveal>
        ...
        <div className="mx-auto max-w-[1600px] px-6 lg:px-12">
          <ScrollReveal>
            <div className="inline-flex items-center gap-2.5 rounded-lg px-3.5 py-1.5" style={{ background: "rgba(27,43,75,0.10)", border: "1px solid rgba(27,43,75,0.20)" }}>
              <span className="font-mono-label text-[10px] tracking-[0.22em] uppercase text-[#243656]">Roles</span>
            </div>
            <h2 className="font-display mt-6 text-[clamp(28px,3.8vw,56px)] leading-[1.15] tracking-[-0.02em] text-[#0B1029]">Current openings</h2>
          </ScrollReveal>
          <Accordion type="single" collapsible className="mt-12 max-w-3xl">
            {departments.map((d, i) => (
              <AccordionItem key={i} value={`dept-${i}`} className="border-b border-dashed border-[#C5CDE0]/70">
                <AccordionTrigger className="font-heading text-lg font-semibold text-[#0B1029]">{d.name}</AccordionTrigger>
                <AccordionContent>
                  <ul className="space-y-3">
                    {d.roles.map((r) => (
                      <li key={r} className="flex items-center justify-between rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                        <span className="font-body text-[13px] text-[#0B1029] font-light">{r}</span>
                        <span className="font-heading text-[13px] font-semibold text-[#E8670A] cursor-pointer hover:underline">Apply →</span>
                      </li>
                    ))}
                  </ul>
                </AccordionContent>
          ...
          <p className="font-body mt-8 text-[13px] text-[#555] font-light">Prefer a general intro? <a href="mailto:info@longentis.com" className="text-[#E8670A] hover:underline">Contact us.</a></p>
        </div>
      </section>
    </main>
    <Footer />
